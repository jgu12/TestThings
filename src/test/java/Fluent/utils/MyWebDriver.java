...
public class MyWebDriver {
    ...
    public MyWebElement waitForElementVisible(String elementName, By by){
        WebElement element;
        try {
            logger.info(String.format("Waiting for element to be visible: [%s], xpath= '%s'", elementName,by.toString()));
            element = wait.until(ExpectedConditions.visibilityOfElementLocated(by));
        } catch (Exception e){
            throw new RuntimeException(
                    String.format("Failed to locate element '%s' by xpath '%s'", elementName, by.toString()));
        }
        if (element == null){
            throw new NullPointerException(String.format("element '%s' is null", elementName));
        }
